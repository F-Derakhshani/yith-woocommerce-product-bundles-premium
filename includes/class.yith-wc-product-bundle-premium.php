...
if ( !class_exists( 'WC_Product_Yith_Bundle' ) ) {
    ...
    class WC_Product_Yith_Bundle extends WC_Product {
        ...
        public function __construct( $product ) {
            ...
            $this->out_of_stock_sync = get_option( 'yith-wcpb-bundle-out-of-stock-sync', 'no' ) === 'yes';
        ...
        public function get_per_item_price_tot_max( $apply_discount = false, $include_optionals = false, $with_max_quantity = false ) {
            ...
            if ( $this->per_items_pricing ) {
                ...
                foreach ( $bundled_items as $bundled_item ) {
                    ...
                    if ( !$bundled_item->has_variables() ) {
                        // SIMPLE
                        $regular_price = yit_get_display_price( $product, $product->get_regular_price() );
                ...
                foreach ( $bundled_items as $bundled_item ) {
                    ...
                    if ( $product->is_type( 'variable' ) ) {
                        if ( isset( $array_var[ $loop ] ) && $variation = wc_get_product( $array_var[ $loop ] ) ) {
                            ...
                            $regular_price = yit_get_display_price( $product, current( $prices[ 'regular_price' ] ) );
                            unset( $prices );
                        }
                    } else {
                        $regular_price = yit_get_display_price( $product, $product->get_regular_price() );
                    ...
                    $qty = !empty( $array_quantity[ $loop ] ) ? $array_quantity[ $loop ] : max( 1, $bundled_item->min_quantity );
