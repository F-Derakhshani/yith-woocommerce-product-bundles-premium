...
class YITH_WCPB_Request_A_Quote_Compatibility {
    ...
    private function __construct() {
        ...
        add_filter( 'ywraq_add_item', array( $this, 'add_bundle_to_quote' ), 10, 2 );
        ...
        add_filter( 'yith_wcpb_after_bundle_woocommerce_get_cart_item_from_session_bundled_by', array( $this, 'bundled_by_from_session' ) );
            ...
            if ( $product && $product->is_type( 'yith_bundle' ) ) {
                ...
                if ( $bundled_items ) {
                    foreach ( $bundled_items as $item ) {
                        ...
                        $quantity = isset( $bundle_info[ 'yith_bundle_quantity_' . $id ] ) ? $bundle_info[ 'yith_bundle_quantity_' . $id ] : 1;
        ...
        if ( isset( $cart_item[ 'bundled_by' ] ) ) {
            ...
            if ( isset( $cart_contents[ $bundle_cart_key ] ) ) {
                ...
                if ( $parent->per_items_pricing == false || isset( $cart_contents[ $bundle_cart_key ][ 'ywraq_price' ] ) ) {
                    $cart_item[ 'data' ]->price = isset( $cart_item[ 'ywraq_price' ] ) ? $cart_item[ 'ywraq_price' ] : 0;
                }
            }
        }
    ...
    public function add_order_item_meta( $values, $cart_item_key, $item_id ) {
        YITH_WCPB_Frontend()->woocommerce_add_order_item_meta( $item_id, $values, $cart_item_key );
