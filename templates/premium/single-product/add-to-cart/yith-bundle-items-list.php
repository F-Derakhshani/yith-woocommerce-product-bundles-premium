...
if ( $bundled_items ) {
    ...
    foreach ( $bundled_items as $bundled_item ) {
        ...
        if ( $hidden ) {
            if ( $bundled_item->has_variables() ) {
                ...
                foreach ( $default_selection as $key => $value ) {
                    $default_selection_tmp[ 'attribute_' . $key ] = YITH_WCPB_WPML_Compatibility::get_wpml_term_slug_current_language( $value, $key );
                    $default_selection[ $key ]                    = YITH_WCPB_WPML_Compatibility::get_wpml_term_slug_current_language( $value, $key );
                }
        ...
        ?>
        <tr class="product yith-wcpb-product-bundled-item" data-is-purchasable="<?php echo $bundled_product->is_purchasable() ? '1' : '0' ?>">
                            ...
                            foreach ( $b_attributes as $name => $options ) : $loop++; ?>
                                <tr>
                                    ...
                                    <td class="value"><select data-type="select" class="yith-wcpb-select-for-variables"
                                            ...
                                            <option
                                                value=""><?php echo __( 'Choose an option', 'woocommerce' ) ?>&hellip;</option>
                                            <?php
                                            if ( is_array( $options ) ) {
                                                ...
                                                if ( taxonomy_exists( $name ) ) {
                                                    if ( !!$selected_value ) {
                                                        $selected_value = YITH_WCPB_WPML_Compatibility::get_wpml_term_slug_current_language( $selected_value, $name );
                                                    }
